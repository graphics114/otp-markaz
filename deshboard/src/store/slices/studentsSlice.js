...
const studentsSlice = createSlice({
  ...
  reducers: {
    ...
    deleteResultSuccess(state, action) {
      state.loading = false;
      state.results = state.results.filter(result => result.result_id !== action.payload);
...
export const fetchAllExamResults = (page = 1) => async (dispatch) => {
...
export const updateResult = (resultId, data, showToast = true) => async (dispatch) => {
  dispatch(studentsSlice.actions.updateResultRequest());

  try {
    const res = await axiosInstance.put(`/exam/update/result/${resultId}`, data);
    dispatch(studentsSlice.actions.updateResultSuccess(res.data.result));

    if (showToast) {
      toast.success(res.data.message || "Result updated");
    }
  } catch (error) {
...
export const addExamResult = (studentId, data) => async (dispatch) => {
  dispatch(studentsSlice.actions.getAllStudentsResultRequest());

  try {
    await axiosInstance.post(`/exam/add/result/${studentId}`, data);

    toast.success("New exam result added");

    // Reload full result list
    dispatch(fetchAllExamResults());
  } catch (error) {
    dispatch(studentsSlice.actions.getAllStudentsResultFailed());
    toast.error(
      error?.response?.data?.message || "Failed to add exam result"
    );
  }
};

export const searchResultsByDate = (from, to) => async (dispatch) => {
  dispatch(studentsSlice.actions.getAllStudentsResultRequest());

  try {
    const res = await axiosInstance.get(
      `/exam/search?from=${from}&to=${to}`
    );
...
export const deleteResult = (resultId) => async (dispatch) => {
  dispatch(studentsSlice.actions.deleteResultRequest());
  try {
    const res = await axiosInstance.delete(`/exam/delete/result/${resultId}`);
    dispatch(studentsSlice.actions.deleteResultSuccess(resultId));
    toast.success(res.data.message || "Result deleted successfully");
  } catch (error) {
    dispatch(studentsSlice.actions.deleteResultFailed());
